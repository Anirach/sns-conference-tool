...
Dev-mode overrides: `VERIFICATION_DEV_MODE=true` makes the email TAN always `123456`. `sns.dev.seed-events=true` (default) seeds the three demo events (NeurIPS Bangkok, ACL Vienna, ICML Montreal expired) idempotently. `sns.dev.seed-demo-data=true` (default off; flipped on in `infra/docker-compose.dev.yml`) additionally seeds 20 fixture users + profiles + participations + interests + ~163 similarity matches + 19 chat messages so the real backend renders a populated UI — login `you@example.com` / `Demo!2026` (also seeded as `SUPER_ADMIN`). The same flag activates `DemoDataKeepalive`, a `@Scheduled` job that bumps `participations.last_update` every minute so seeded fellows stay inside `VicinityService`'s 5-minute freshness window indefinitely. Refused under `prod` by `ProductionSecretsCheck`. `SNS_ADMIN_EMAIL=you@example.com` (also in compose) drives `AdminBootstrap` to promote that account to `SUPER_ADMIN` at boot — `ProductionSecretsCheck` requires it under `prod`. The `web` container ships with `NEXT_PUBLIC_MOCK_API=0` so it talks to the seeded backend out of the box; flip to `"1"` for MSW-only frontend work. `sns.rate-limit.backend=memory` (default) runs the fixed-window limiter in-process — flip to `redis` for multi-pod prod. `sns.chat.relay` defaults to `redis` (requires Redis on the classpath connection) — flip to `inproc` for tests or local single-node runs. See [backend/README.md](backend/README.md#configuration-reference) for the full property list.
